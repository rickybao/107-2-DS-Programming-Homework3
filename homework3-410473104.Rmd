...
```{r ans11}
# loss1Companies
finStatement[["營業利益（損失）"]] < 0 -> lc1
finStatement[["公司名稱"]][c(lc1)] -> loss1Companies
loss1Companies
```
...
```{r ans12}
# loss2Companies
finStatement[["基本每股盈餘（元）"]] < 0 -> lc2
finStatement[["公司名稱"]][c(lc2)] -> loss2Companies
loss2Companies
```
...
```{r ans13}
# only1LossCompanies
!(loss1Companies %in% loss2Companies) -> olc1
!(loss2Companies %in% loss1Companies) -> olc2
loss1Companies[c(olc1)] -> o1c1
loss2Companies[c(olc2)] -> o1c2
c(o1c1,o1c2) -> only1LossCompanies
only1LossCompanies
```
...
```{r ans14}
# subsample
finStatement[["公司名稱"]] %in% only1LossCompanies -> sbs
finStatement[c(sbs),c(1:33)] -> subsample
subsample
```
...
```{r ans22}
# df_oneShow
exhibitionList[[1]][[3]] -> os1
exhibitionList[[1]][[5]][[1]][[1]] -> os21
exhibitionList[[1]][[5]][[1]][[2]] -> os22
exhibitionList[[1]][[5]][[1]][[3]] -> os23
exhibitionList[[1]][[5]][[1]][[5]] -> os24
exhibitionList[[1]][[5]][[1]][[8]] -> os25
exhibitionList[[1]][[18]] -> os3
list(title=os1, time=os21, location=os22, locationName=os23, price=os24, endTime=os25, sourceWebName=os3) -> os
as.data.frame(os) -> df_oneShow
df_oneShow
```
...
```{r ans25}
# totalCount
map(exhibitionList,~.[[18]]) == "全國藝文活動資訊系統" -> tc
sum(tc) -> totalCount
as.numeric(totalCount) -> totalCount
totalCount
```
